...
**The revision loop.** "Return with questions" is a turn in the discussion, not a dead end: a return with a non-empty note automatically reconvenes the board. Each role receives the returned document in full (once submitted, it is company record), the CEO's questions, and its own prior position; revised positions are written blind to colleagues' *new* positions, and the synthesis must open by answering the CEO's questions directly and say what changed versus what deliberately did not. The new document (`<id>-r2`, `-r3`, ...) references the original via `revises` and awaits signature like any proposal. Nothing self-drives: every round is triggered only by the CEO's next return.

**The Chief of Staff — the process role (`kind: "staff"`).** A role that never writes a board position, never takes a business stance and never votes; its mandate is the CEO's attention and the integrity of the decision process. It does two jobs. **Clarification triage:** before a question reaches the board, one cheap call (≈500 tokens, temperature 0.2) decides whether the board can answer it as asked or whether the CEO must first supply something only the CEO knows — intent, constraints, appetite. Anything the dossier holds or a context tool could fetch is the board's work and must never be asked of the CEO. A hold appends `clarification_requested {id, questionEventId, questionText, byRoleId, questions}` and stops the run there — no positions, no document, one call spent — until `POST /api/companies/:id/clarifications/:clarificationId/answer` appends `clarification_answered` and resumes the run with the Q&A carried into every prompt as the highest authority. One round, at most three questions, and a triage that fails or times out always *proceeds* (a broken gate must never cost a deliberation). **Synthesis authorship:** the proposal document is written by the Chief of Staff rather than by a board member, which removes the author-judges-their-own-position bias — the same synthesis machinery, a voice that collects positions and records disagreements without adding an opinion. A company whose config has no `staff` role skips both: no triage call, and a board member signs the document exactly as before.
