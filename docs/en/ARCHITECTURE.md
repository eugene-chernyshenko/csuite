...
**Board context — on demand, not wholesale.** A company holds a lot of data (dossier, ADRs, balances and financial summaries, tasks, metrics, priorities, the decision log), and not all of it is relevant to every question. Stage 1 (now): a static dossier in the company config — values, key numbers, priorities — enters the position prompt as grounded fact. Stage 2: tool use — the position agent queries what it needs through the platform API (`get_financials`, `get_metrics`, `list_decisions`, `read_adr`, ...) and the asset registry; what a role chose to consult is recorded as events and lands in the position's provenance.
